...
import { Button, H1, Paragraph, Text, XStack, YStack } from '@my/ui'
...
export default function LandingScreen() {
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" style={{ flex: 1 }}>
      <YStack flex={1} bg="$background" px="$5" py="$6" gap="$6">
        <XStack justifyContent="space-between" alignItems="center">
          <Text textTransform="uppercase" letterSpacing={3} fontSize="$2" color="$color10">
            Richharbor
          </Text>
          <XStack gap="$2">
            <Button size="$3" onPress={() => router.push('/login')} bg="$color12" color="white">
              Login
            </Button>
            <Button size="$3" onPress={() => router.push('/signup')} bg="$color2" color="$color12">
              Register
            </Button>
          </XStack>
        </XStack>
        ...
        <YStack gap="$3">
          ...
          <XStack gap="$3" flexWrap="wrap">
            <Button onPress={() => router.push('/signup')} bg="$blue10" color="white">
              Register as Customer
            </Button>
            <Button onPress={() => router.push('/signup')} bg="$color2" color="$color12">
              Become a Partner
            </Button>
            <Button onPress={() => router.push('/signup')} bg="$color2" color="$color12">
              Refer Leads
        ...
        <YStack gap="$4" bg="$color1" borderRadius="$8" p="$5" borderWidth={1} borderColor="$color3">
        ...
        <XStack gap="$2" flexWrap="wrap">
          {trustItems.map((item) => (
            <YStack key={item} bg="$color2" borderRadius="$6" px="$3" py="$2">
