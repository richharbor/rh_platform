...
// ---- step 4: activity, documents, notifications, support (mirrors the Expo app) ----
...
export const useTicket = (id: string) =>
  useQuery({ queryKey: ["ticket", id], queryFn: () => api("support.ticket", { id }), refetchInterval: (q) => (q.state.data?.advisorTyping ? 1500 : false) });

// ---- step 5: private markets (mirrors the Expo app) ----

export const useCompany = (id: string) => useQuery({ queryKey: ["company", id], queryFn: () => api("companies.get", { id }) });
...
export const useListings = () =>
  useQuery({ queryKey: ["listings"], queryFn: () => api("listings.list", undefined), refetchInterval: (q) => (q.state.data?.some((l) => !["paid", "cancelled"].includes(l.state)) ? 3000 : false) });

export const useListing = (id: string) =>
  useQuery({ queryKey: ["listing", id], queryFn: () => api("listings.get", { id }), refetchInterval: (q) => (q.state.data && !["paid", "cancelled"].includes(q.state.data.state) ? 2000 : false) });
...
export const useBenefits = () =>
  useQuery({ queryKey: ["benefits"], queryFn: () => api("benefits.list", undefined), staleTime: 0, refetchInterval: (q) => (q.state.data?.some((b) => b.state === "processing") ? 2000 : false) });
...
export const useReferrals = () =>
  useQuery({ queryKey: ["referrals"], queryFn: () => api("referrals.list", undefined), staleTime: 0, refetchInterval: (q) => (q.state.data?.some((r) => r.state === "approved") ? 3000 : false) });

// ---- step 7: partner (mirrors the Expo app) ----
...
// ---- step 8: Customer 360, goals, family, insights, assistant (mirrors the Expo app) ----
