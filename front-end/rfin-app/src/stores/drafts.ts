...
/** Half-finished applications, keyed by product — resumable (report #13, UX rule 8). */
...
export const useDrafts = create<State>()(
  persist(
    (set) => ({
      ...
      save: (id, patch) =>
        set((s) => ({
          drafts: {
            ...s.drafts,
            [id]: { ...(s.drafts[id] ?? { step: 0, amount: 0, consents: [] }), ...patch, updatedAt: new Date().toISOString() },
          },
        })),
      clear: (id) =>
        set((s) => {
    ...
    { name: "rfin.drafts", storage: createJSONStorage(() => AsyncStorage) },
  ),
);
