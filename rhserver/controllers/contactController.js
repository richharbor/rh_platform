...
// Ported unchanged (per plan) from tps-next-backend's
// controllers/campaign/upload-conctacts.controller.js, aside from the
// `../../models` -> `../models` require path.
  ...
  try {
    ...
    if (contactListId) {
      ...
      if (!listName) {
        return res
          .status(400)
          .json({ message: "Contact list name is required" });
      }
