...
export default function BlogForm({
  ...
  // Backend normalizes "publish" -> stored "published"; kept as "publish"
  // here to match the source's request-body contract for this v1 form.
  ...
  // Cast at the API boundary: this v1 form's request-body shape (readTime as
  // number, structured content blocks incl. youtube) is the actual payload
  // the backend stores as opaque JSON — it doesn't need to satisfy the v2
  // service module's separately-shaped `BlogData` type.
  const updateBlogDataFn = async (requestbody: ReturnType<typeof prepareRequestBody>) => {
    if (!id) return;
    try {
      const response = await updateBlog(
        requestbody as unknown as Parameters<typeof updateBlog>[0],
        id,
      );
  ...
  const addBlogDataFn = async (requestbody: ReturnType<typeof prepareRequestBody>) => {
    try {
      ...
      <div className="container mx-auto pb-10 px-10 py-5">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <FormLabel className="text-base">FAQs (optional)</FormLabel>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
              ...
              {faqFields.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  No FAQs added. These render as an accordion on the blog
                  page.
                </p>
              )}
